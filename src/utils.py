...
class Logger:
    ...
    def save_model(self,model, epoch):
        ...
        if self.metrics['valid_loss'][-1] == torch.min(torch.stack(self.metrics['valid_loss'])):#np.min(self.metrics['valid_loss']):
            torch.save(model.module.state_dict(), os.path.join(save_path,'best_loss.pth'))
        if self.metrics['valid_top1'][-1] == torch.max(torch.stack(self.metrics['valid_loss'])):#np.max(self.metrics['valid_top1']):
